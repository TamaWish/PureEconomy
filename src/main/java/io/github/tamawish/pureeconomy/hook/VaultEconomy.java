...
/** Vault {@link net.milkbowl.vault.economy.Economy} adapter for PureEconomy's default currency. */
public final class VaultEconomy extends AbstractEconomy {

  private final PureEconomy plugin;

  /**
   * Creates the Vault adapter.
   *
  ...
  private EconomyResponse withdraw(UUID uuid, double amount) {
    ...
    BigDecimal value = cur().normalize(BigDecimal.valueOf(amount));
    if (value.compareTo(BigDecimal.ZERO) <= 0) {
      return fail("Amount is below the minimum currency precision");
    }
    if (!eco().take(uuid, cur(), value)) {
      return new EconomyResponse(
          0,
          eco().get(uuid, cur()).doubleValue(),
  ...
  public boolean createPlayerAccount(String playerName) {
    ...
    if (uuid == null) {
      ...
      eco().ensureAccount(offline.getUniqueId(), playerName);
      return true;
    }
