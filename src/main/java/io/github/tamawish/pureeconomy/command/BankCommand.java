...
/** Handles {@code /bank} balance, deposit, and withdraw actions. */
public final class BankCommand implements CommandExecutor, TabCompleter {
  ...
  private void sendBalance(Player player, Currency currency) {
    ...
    plugin
        .lang()
        .send(player, "bank-balance", Lang.of("currency", currency.name(), "amount", amount));
  }
      ...
      plugin
          ...
          .send(
              ...
              Lang.of(
                  ...
                  currency.name(),
                  "amount",
                  currency.format(plugin.economy().getBank(player.getUniqueId(), currency))));
    }
  }
  ...
  public List<String> onTabComplete(
    ...
    if (args.length == 2
        && ("balance".equalsIgnoreCase(args[0]) || "bal".equalsIgnoreCase(args[0]))) {
