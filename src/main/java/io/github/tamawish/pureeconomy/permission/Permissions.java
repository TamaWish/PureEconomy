...
public final class Permissions {
  ...
  /**
   * Creates a permission resolver bound to the plugin config.
   *
   ...
   *
   * @param node logical permission slot
   * @return permission node string
   */
  public String name(Node node) {
    String value =
        plugin.getConfig().getString("permissions." + node.configKey + ".node", node.fallbackNode);
   ...
   * @param node logical permission slot
   * @return {@code true} when the sender is allowed
   */
  ...
  /**
   * Parses a config default string into a Bukkit {@link PermissionDefault}.
   *
   * @param raw config value such as {@code everyone}, {@code op}, or {@code nobody}
   * @param fallback value used when {@code raw} is blank or unrecognized
